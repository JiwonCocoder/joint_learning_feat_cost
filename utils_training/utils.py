...
def load_checkpoint(model, optimizer, scheduler, filename='checkpoint.pth.tar'):
    ...
    if os.path.isfile(filename):
        ...
        pdb.set_trace()
        if filename =='GLUNet_GOCor_star_megadepth.pth':
            net_name = 'pyramid_256.'
        else:
            net_name = 'pyramid.'
        ...
        for k in list(state_dict.keys()):
            if k.startswith(net_name):
                # new_k = k.replace("pyramid", "feature_extraction.backbone")
                # state_dict[new_k] = state_dict[k]
                state_dict[k[len(net_name):]] = state_dict[k]
            del state_dict[k]
