...
class TransformerAggregator(nn.Module):
    def __init__(self, num_hyperpixel, img_size=224, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4., qkv_bias=True,
                 qk_scale=None,
...
class FeatureExtractionHyperPixel(nn.Module):
    def __init__(self, hyperpixel_ids, feature_size, freeze=True, use_adap = True):
        super().__init__()
        self.backbone = vgg.VGGPyramid(train=False)
    ...
    def forward(self, img):
        ...
        if self.use_adap:
            feats[-1] = self.adap_layer_feat3(feats[-1])
        feat = F.interpolate(feats[-1], self.feature_size, None, 'bilinear', True)
        return [feat]
...
class CATs(nn.Module):
    def __init__(self,
        ...
        channels = [512]
        self.use_adap = use_adap
        self.feature_extraction = FeatureExtractionHyperPixel(hyperpixel_ids, feature_size, freeze, self.use_adap)
        self.proj = nn.ModuleList([
            nn.Linear(channels[0], self.feature_proj_dim) for i in hyperpixel_ids
        ])
    ...
    def forward(self, target, source, mode='train'):
        ...
        for i, (src, tgt) in enumerate(zip(src_feats, tgt_feats)):
            ...
            # corr_3d_S_Tvec_jw = corr_4d_S_Tvec_jw.view(corr_2d_S_Tvec.size(0),
            #                                      self.feature_size * self.feature_size,
            #                                      self.feature_size, self.feature_size)
            # corr_3d_T_Svec_jw = corr_4d_S_Tvec_jw.view(corr_2d_S_Tvec.size(0),
            #                                     self.feature_size, self.feature_size,
        ...
        # 1D
        scores_S_Tvec, index_S_Tvec = torch.max(self.l2norm(corr_feat), dim=1)
        scores_T_Svec, index_T_Svec = torch.max(self.l2norm(corr_feat.transpose(-2, -1)), dim=1)
        # 2D
        index_S_Tvec = index_S_Tvec.view(index_S_Tvec.size(0), self.feature_size, self.feature_size)
        index_T_Svec = index_T_Svec.view(index_T_Svec.size(0), self.feature_size, self.feature_size)
        # occ
        ...
        # Agg_corr#
        # 1D
        refined_scores_S_Tvec, refined_index_S_Tvec = torch.max(self.l2norm(refined_corr), dim=1)
        refined_scores_T_Svec, refined_index_T_Svec = torch.max(self.l2norm(refined_corr.transpose(-2, -1)), dim=1)
        # 2D
        refined_index_S_Tvec = refined_index_S_Tvec.view(refined_index_S_Tvec.size(0), self.feature_size,
                                                         self.feature_size)
        refined_index_T_Svec = refined_index_T_Svec.view(refined_index_T_Svec.size(0), self.feature_size,
                                                         self.feature_size)
        ...
        if self.count % 20 == 0:
            ...
            fig = self.plot_test_map_mask_img(target[0].unsqueeze(0), source[0].unsqueeze(0),
                                              refined_index_S_Tvec[0].unsqueeze(0),
                                              refined_index_T_Svec[0].unsqueeze(0),
        ...
        loss_S_Tvec_feat_by_feat = self.calc_pixelCT_mask(corr_feat, index_S_Tvec, occ_S_Tvec, self.temperature,
                                                          self.neg_all, self.use_mask)
        # loss_S_Tvec_feat_by_agg = self.calc_pixelCT_mask(corr_feat, refined_index_S_Tvec, refined_occ_S_Tvec, self.temperature, self.norm_type)
        # loss_S_Tvec_agg_by_feat = self.calc_pixelCT_mask(refined_corr, index_S_Tvec, occ_S_Tvec, self.temperature, self.norm_type)
        # loss_S_Tvec_agg_by_agg = self.calc_pixelCT_mask(refined_corr, refined_index_S_Tvec, refined_occ_S_Tvec, self.temperature, self.norm_type)

        if mode == 'train':
            return [loss_S_Tvec_feat_by_feat]
        elif mode == 'eval':
            grid_x, grid_y = self.soft_argmax(corr.squeeze(0).view(B, -1, self.feature_size, self.feature_size))
